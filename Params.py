...
@dataclass
class Params:
    tau_NMDA = 60*ms  # E synaptic time constants (ms)
    ...
    I_bg_E = 329.4*pA      # Background inputs (pA)
    I_bg_I = 260*pA        # Background inputs (pA)
    a = 0.135*(Hz/pA)           # f-I curve (E cells) (Hz/pA)
    b = 54*Hz             # f-I curve (E cells) (Hz)
    d= 0.308*second           # f-I curve (E cells) (s)
    beta_i = 153.75*(Hz/nA)        #f-I curve (I cells) (Hz/nA)
    I_th = 252*pA          # f-I curve (I cells) (Hz)
    b1: float = 0.3            # Rescale FLN
    G_n_loc_E_E = 480*pA   # Excitatory strengths NMDA (pA)
    ...
    I_stim = 300*pA       # Stimulus strength (pA)
    I_vig = 125*pA       # Vigilance strength (pA)
